...
import { COOKBOOK_RECORDS, COOKBOOK_RECIPE_INCLUSIONS, COOKBOOK_SECTIONS } from './records';
...
describe('V3C.30 cookbook production audit', () => {
  it('accepts the first canonical cookbook batch as internal production work', () => assert.equal(auditCookbookProduction(COOKBOOK_RECORDS, COOKBOOK_SECTIONS, COOKBOOK_RECIPE_INCLUSIONS).valid, true));
  ...
  it('rejects lifecycle divergence from canonical recipe content', () => assert.equal(auditCookbookProduction([cookbook], [section], [{ ...inclusion, editorialReviewStatus: 'approved' }]).valid, false));
