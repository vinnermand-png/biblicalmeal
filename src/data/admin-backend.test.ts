...
class TestPersistenceProvider implements AdminPersistenceProvider {
  ...
  async readDraft(input: {
    resourceKind: AdminPersistentDraft['resourceKind'];
    canonicalId: string;
  }) {
    return (
      this.drafts.get(`${input.resourceKind}:${input.canonicalId}`) ?? null
    );
  }
...
describe('V3C.37 authenticated admin backend foundation', () => {
  ...
  it('rejects unauthenticated mutations', async () => {
    const service = new AdminBackendService(
      authProvider(null),
      new TestPersistenceProvider(),
    );
    const result = await service.saveDraft({
      principal: null,
      resourceKind: 'article-content',
      canonicalId: 'article:grain',
      canonicalOwnerId: 'article:grain',
      payload: {},
  ...
  it('rejects conflicting canonical ownership and invalid workflow transitions', async () => {
    ...
    const conflict = await service.saveDraft({
      ...
      resourceKind: 'article-content',
      canonicalId: 'article:grain',
      canonicalOwnerId: 'article:other',
      payload: {},
      now: '2026-08-24T00:01:00Z',
    });
    expect(conflict.ok).toBe(false);
    const invalid = await service.transition({
      principal: administrator,
      resourceKind: 'article-content',
      canonicalId: 'article:grain',
      to: 'approved',
