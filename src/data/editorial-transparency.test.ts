...
import { getPublicSourceRegistry, getTransparencySnapshot, TRANSPARENCY_EVIDENCE_LABELS } from './editorial-transparency';
...
describe('V3C.21 trust and editorial transparency', () => {
  it('keeps the public transparency snapshot tied to canonical records', () => {
    ...
    expect(snapshot.evidenceStates).toEqual(expect.arrayContaining(['contextual', 'unresolved']));
  });
  ...
  it('exposes only canonical source metadata and does not fabricate completeness', () => {
    ...
    expect(sources.every((source) => source.id && source.title && source.notes)).toBe(true);
    expect(sources.some((source) => source.reliability === 'unassessed')).toBe(true);
  });
});
