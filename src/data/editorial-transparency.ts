...
export type TransparencyEvidenceLabel =
  'direct' | 'partial' | 'contextual' | 'unresolved';

export const TRANSPARENCY_EVIDENCE_LABELS: Record<
  TransparencyEvidenceLabel,
  string
> = {
  direct: 'Directly supported by the recorded evidence.',
  partial: 'Partially supported; important limits remain.',
  contextual:
    'Provides context but does not establish the full claim on its own.',
...
export const EDITORIAL_TRANSPARENCY = {
  ...
  summary:
    'BiblicalMeal separates scripture, historical evidence, interpretation, reconstruction, and modern adaptation instead of presenting them as the same thing.',
...
export function getTransparencySnapshot() {
  const evidenceStates = new Set(
    CITATION_RECORDS.map((citation) => citation.evidenceState),
  );
  return {
