...
export function getRankingOptimizationCandidate(
  ...
  return RANKING_OPTIMIZATION_CANDIDATES.find((candidate) => candidate.targetId === targetId);
}
...
export function auditRankingOptimizationSystem(
  ...
  for (const candidate of candidates) {
    if (!candidate.targetId.trim()) issues.push('Missing canonical optimization target ID.');
    if (candidateIds.has(candidate.targetId)) {
      issues.push(`Duplicate canonical optimization target: ${candidate.targetId}.`);
    }
    candidateIds.add(candidate.targetId);

    if (!candidate.canonicalRoute.startsWith('/')) {
      issues.push(`Optimization candidate route is not canonical: ${candidate.targetId}.`);
    }
    if (candidateRoutes.has(candidate.canonicalRoute)) {
      issues.push(`Duplicate optimization candidate route: ${candidate.canonicalRoute}.`);
    }
    candidateRoutes.add(candidate.canonicalRoute);
  }

  for (const record of records) {
    if (!record.id.trim()) issues.push('Missing ranking optimization record ID.');
    ...
    if (!candidate || !candidateIds.has(record.targetId)) {
      issues.push(`Optimization record references unknown canonical target: ${record.id}.`);
    } else if (record.canonicalRoute !== candidate.canonicalRoute) {
      issues.push(`Optimization record route does not match canonical target: ${record.id}.`);
    }

    if (record.types.length === 0) {
      issues.push(`Optimization record has no optimization type: ${record.id}.`);
    }
    if (record.evidence.length === 0) {
      issues.push(`Optimization record has no documented evidence: ${record.id}.`);
    }
    if (record.history.length === 0) {
      issues.push(`Optimization record has no history: ${record.id}.`);
    }
    if (record.requiresExistingPublicationGate !== true) {
      issues.push(`Optimization record bypasses existing publication gates: ${record.id}.`);
    }
    ...
    for (const evidence of record.evidence) {
      ...
      if (!evidence.summary.trim()) {
        issues.push(`Optimization evidence is missing a summary: ${record.id}.`);
      }
      if (!/^\d{4}-\d{2}-\d{2}$/.test(evidence.observedOn)) {
        issues.push(`Optimization evidence date must be ISO calendar format: ${record.id}.`);
      }
    }
    ...
    if (
      ...
      issues.push(`Optimization cannot reach publication hand-off without a documented opportunity: ${record.id}.`);
    }
  }
