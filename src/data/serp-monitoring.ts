...
export type SerpMeasurementSource =
  'google-search-console' | 'serp-provider' | 'manual-verification';
...
export const SERP_MONITORING_TARGETS: readonly SerpMonitoringTarget[] =
...
export function getSerpMonitoringTarget(
  targetId: string,
): SerpMonitoringTarget | undefined {
...
export function getSerpTargetStatus(
  ...
  if (!latestSnapshot) {
    return {
      ...
      notes:
        'No real SERP or Search Console measurement has been imported yet.',
    };
  }
...
export function auditSerpMonitoring(
  ...
  for (const target of targets) {
    if (!target.targetId.trim())
      issues.push('Missing canonical SEO target ID.');
    if (targetIds.has(target.targetId))
      issues.push(`Duplicate SERP target ID: ${target.targetId}.`);
  ...
  for (const snapshot of snapshots) {
    if (!targetIds.has(snapshot.targetId)) {
      issues.push(
        `Snapshot references unknown SEO target: ${snapshot.targetId}.`,
      );
    }
    if (!/^\d{4}-\d{2}-\d{2}$/.test(snapshot.observedOn)) {
      issues.push(
        `Snapshot date must be ISO calendar format: ${snapshot.targetId}.`,
      );
    }
    for (const measurement of snapshot.measurements) {
      if (
        measurement.canonicalRoute !==
        getSerpMonitoringTarget(snapshot.targetId)?.canonicalRoute
      ) {
        issues.push(
          `Measurement route does not match canonical target: ${snapshot.targetId}.`,
        );
      }
      ...
      if (measurement.position !== undefined && measurement.position <= 0) {
        issues.push(
          `Position must be greater than zero: ${snapshot.targetId}.`,
        );
      }
      if (
      ...
      if (
        ...
        issues.push(
          `Average CTR must be between 0 and 1: ${snapshot.targetId}.`,
        );
      }
    }
  }
