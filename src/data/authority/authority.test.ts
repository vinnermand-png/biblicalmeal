...
describe('V3C.20 authority and citation expansion', () => {
  ...
  it('supports traceability across research, editorial and recipe targets without publishing them', () => {
    expect(CITATION_RECORDS.some((x) => x.targetKind === 'research-dossier')).toBe(true);
    expect(CITATION_RECORDS.some((x) => x.targetKind === 'article-content')).toBe(true);
    expect(CITATION_RECORDS.some((x) => x.targetKind === 'recipe-research')).toBe(true);
    expect(CITATION_RECORDS.some((x) => x.targetKind === 'recipe-content')).toBe(true);
  });
  it('detects duplicate identities and invalid canonical references', () => {
    ...
    expect(audit.issues.map((x) => x.code)).toEqual(expect.arrayContaining(['duplicate-authority-id','duplicate-citation-id','invalid-authority-reference','invalid-target-reference','citation-traceability-gap']));
  });
  it('rejects overclaiming, invalid direct support and hidden uncertainty', () => {
    const invalid: CitationRecord = { ...CITATION_RECORDS[0], id: 'citation-invalid', relationship: 'directly-supports', evidenceState: 'unresolved', verificationState: 'verified', claimStrength: 'direct', uncertaintyDisclosure: '' };
