...
const workflowItems: AdminWorkflowItem[] = [
  ...ARTICLE_CONTENT_RECORDS.map((record) => ({
    ...
    blockers: record.publicationEligible ? [] : ['Existing research and editorial publication gates remain authoritative.'],
  })),
  ...
  ...COOKBOOK_RECORDS.map((record) => ({
    ...
    blockers: record.publicationEligible ? [] : ['Cookbook production does not promote draft recipes or bypass recipe publication gates.'],
  })),
];
...
export function evaluateAdminMutation(_attempt: AdminMutationAttempt): AdminMutationDecision {
  return {
    allowed: false,
    reason: 'V3C.31 exposes canonical status and readiness only. This MVP has no mutation or publication authority and cannot bypass canonical gates.',
  };
}
