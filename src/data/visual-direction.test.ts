...
describe('V3D.1 visual direction', () => {
  it('preserves the canonical BiblicalMeal identity statement', () => {
    expect(VISUAL_DIRECTION.identity).toBe('Ancient roots. Modern editorial design.');
  });
  ...
  it('keeps motion restrained and reduced-motion aware', () => {
    expect(VISUAL_DIRECTION.motion.reducedMotion).toContain('prefers-reduced-motion');
  ...
  it('defines a non-generic illustration direction and explicit exclusions', () => {
    expect(VISUAL_DIRECTION.illustrationDirection).toContain('clear silhouette');
