...
if (!articleIds.has('article-biblical-food-evidence-labels')) {
  throw new Error(
    'Instagram content requires canonical source content before social drafts can be created.',
  );
}
if (!websiteSource) {
  throw new Error(
    'V3C.34 prototype requires the canonical V3C.32 website-content record.',
  );
}
...
const prototypeAuthorityIds = websiteSource.citationIds
  .map(
    (citationId) =>
      CITATION_RECORDS.find((citation) => citation.id === citationId)
        ?.authorityId,
  )
  .filter(
    (id): id is string =>
      Boolean(id) && AUTHORITY_RECORDS.some((authority) => authority.id === id),
  );
...
export const INSTAGRAM_CONTENT_RECORDS: readonly InstagramContentRecord[] = [
  {
    ...
    title:
      'How BiblicalMeal Separates Evidence, Inference and Editorial Content',
    mode: 'factual',
    status: 'draft',
    canonicalSources: [
      { kind: 'article-content', id: 'article-biblical-food-evidence-labels' },
    ],
    sourceImageAssetIds: [],
    brief: {
      objective:
        'Explain the project methodology without converting methodology into new historical claims.',
      audienceIntent:
        'Understand how BiblicalMeal labels supported, inferred, editorial and unresolved material.',
      factualBoundaries: [
    ...
    draft: {
      ...
      caption:
        'BiblicalMeal keeps supported evidence, inference, editorial explanation and unresolved questions distinct. A clear format can make research easier to follow, but formatting itself does not turn a claim into verified research.',
      carousel: [],
      reelScript: [],
      visualDirection:
        'Clean editorial card explaining the four evidence boundaries; no documentary or historical-scene claim is implied.',
      visualBrief: 'Editorial social card; no documentary evidence claim.',
      hashtags: ['#BiblicalMeal', '#BiblicalResearch', '#FoodHistory'],
      platformNotes: [
        'Educational editorial draft only.',
        'Do not imply external publication approval.',
      ],
    },
  ...
  },
  {
    ...
    brief: {
      objective:
        'Create a reflective social concept without presenting inspiration as historical verification.',
      audienceIntent:
        'Engage with the project theme through a clearly non-factual reflective format.',
      factualBoundaries: ['Do not introduce factual historical claims.'],
      uncertaintyBoundaries: [
        'Inspiration must not resolve or conceal research uncertainty.',
      ],
      disclosureRequirements: [
        'Keep reflective wording distinct from evidence labels.',
      ],
    },
    draft: {
      hook: 'A meal can invite reflection without becoming a historical claim.',
      caption:
        'BiblicalMeal explores food, context and evidence with room for reflection. Reflection can be meaningful, but it should never be confused with research verification.',
      carousel: [],
      reelScript: [],
      visualDirection:
        'Quiet editorial food detail or existing illustrative asset; avoid documentary claims about ancient events.',
      visualBrief:
        'Illustrative reflective direction; not documentary evidence.',
  ...
  },
  {
    ...
    sourceImageAssetIds: [
      'ai-editorial-ai-website-article-figs-research-context',
    ],
    brief: {
      objective:
        'Adapt only the canonical V3C.32 figs research context into an evidence-bound Instagram explanation.',
      audienceIntent:
        'Understand the evidence scope without mistaking editorial formatting for new research.',
      factualBoundaries: [
        'Use only the canonical website-content source relationships and cited evidence already resolved by V3C.32.',
        'Do not convert the separate sycomore uncertainty into a common-fig conclusion.',
      ],
      uncertaintyBoundaries: [
        'The social draft preserves the source uncertainty rather than resolving sycomore identification or adding new historical claims.',
      ],
      disclosureRequirements: [
        'Social formatting is an editorial summary of canonical evidence-bound website content.',
        'The related image is editorial/illustrative and not documentary historical evidence.',
      ],
    },
    draft: {
      ...
      visualDirection:
        'Use the V3C.33 editorial image direction connected to the same V3C.32 website content record.',
      visualBrief:
        'Editorial/illustrative social visual derived from the canonical website image brief; not documentary historical evidence.',
      hashtags: ['#BiblicalMeal', '#BiblicalResearch', '#FoodHistory'],
      platformNotes: [
        'Internal V3C.34 prototype.',
        'No Instagram API integration.',
        'No external publication approval or automatic publishing.',
      ],
    },
    evidenceReferences: [
      websiteSource.sourceArticleContentId,
      ...websiteSource.researchDossierIds,
    ],
